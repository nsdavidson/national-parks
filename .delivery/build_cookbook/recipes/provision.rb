...
ruby_block 'get-elb' do
  block do
    node.run_state['elb'] = Mixlib::ShellOut.new("kubectl get service nationalparks-#{node['delivery']['change']['stage']} -o json | jq '.status.loadBalancer.ingress[0].hostname' -r").stdout.chomp
  end
  action :run
end
